    ...
    if !harness.hostWorkspaces.isEmpty {
        ...
        ForEach(harness.hostWorkspaces) { ws in
          HStack(spacing: 8) {
            Label(ws.title, systemImage: "folder")
              ...
              .lineLimit(1)
            Spacer()
            Button(action: { title = ws.title; renameTarget = ws }) {
              Image(systemName: "pencil")
            }
            .buttonStyle(.borderless)
            .help("重命名工作区")
            Button(action: { harness.deleteWorkspace(ws) }) {
      ...
      .sheet(item: $renameTarget) { ws in
        VStack(alignment: .leading, spacing: 14) {
          Text("重命名工作区").font(.title3.weight(.bold))
          TextField("名称", text: $title)
          HStack {
            Spacer()
            Button("取消") { renameTarget = nil }
            Button("保存") {
              harness.renameWorkspace(ws, title: title)
              renameTarget = nil
            }
