  ...
  private func terminal(_ view: HarnessController.ToolPresentation) -> some View {
    Card(icon: "terminal", title: view.title ?? "终端") {
      VStack(alignment: .leading, spacing: 8) {
        if let description = view.description { Text(description).font(.caption).foregroundStyle(.secondary) }
        if let cwd = view.cwd { Label(cwd, systemImage: "folder").font(.caption.monospaced()).foregroundStyle(.secondary) }
        HStack(spacing: 7) { Text("$").foregroundStyle(.mint); Text(view.title ?? "").textSelection(.enabled) }
          .font(.system(.caption, design: .monospaced).weight(.medium))
        Divider()
        code(view.output ?? tool.output)
      }
    } trailing: {
      if let exitCode = view.exitCode { Badge(text: "exit \(exitCode)", color: exitCode == 0 ? .green : .red) }
      else if let signal = view.signal { Badge(text: signal, color: .orange) }
      ...
      if view.diffs.isEmpty { code(view.output ?? "没有可展示的差异。") }
        ...
        VStack(alignment: .leading, spacing: 11) {
          ForEach(view.diffs) { item in
            ...
            }.padding(8).background(.primary.opacity(0.035), in: RoundedRectangle(cornerRadius: 7))
          }
        }
      }
    }
  ...
  private func read(_ view: HarnessController.ToolPresentation) -> some View {
    Card(icon: "doc.text", title: view.title ?? view.path ?? "读取文件") {
      VStack(alignment: .leading, spacing: 7) {
        ...
        if let total = view.totalLines { Text("显示 \(view.lines.count) / \(total) 行\(view.lang.map { " · \($0)" } ?? "")").font(.caption2).foregroundStyle(.secondary) }
        if view.lines.isEmpty { code(view.output ?? "文件为空。") }
        else {
          VStack(alignment: .leading, spacing: 0) {
            ForEach(view.lines) { line in
              HStack(alignment: .top, spacing: 9) {
                Text("\(line.number)").frame(minWidth: 34, alignment: .trailing).foregroundStyle(.tertiary)
          ...
          }.padding(8).background(.primary.opacity(0.045), in: RoundedRectangle(cornerRadius: 7))
        }
      }
    }
  }
  ...
  private func search(_ view: HarnessController.ToolPresentation) -> some View {
    Card(icon: "magnifyingglass", title: view.title ?? "搜索结果") {
      VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 3) {
              ...
              ForEach(file.matches) { match in
                HStack(alignment: .top, spacing: 8) {
                  Text("\(match.lineNumber)").frame(minWidth: 30, alignment: .trailing).foregroundStyle(.tertiary)
        ...
        if view.truncated { Badge(text: view.total.map { "已显示部分结果（共 \($0) 条）" } ?? "已显示部分结果", color: .orange) }
      }
    }
  }
    ...
    Card(icon: "globe", title: view.title ?? (view.webKind == "fetch" ? "网页抓取" : "网页搜索")) {
      VStack(alignment: .leading, spacing: 9) {
        if view.webKind == "fetch", let url = view.url {
          ...
          if let statusCode = view.statusCode { Badge(text: "HTTP \(statusCode)", color: (200...299).contains(statusCode) ? .green : .orange) }
        } else {
          ...
          ForEach(view.sources) { source in
            VStack(alignment: .leading, spacing: 3) {
              ...
              if let snippet = source.snippet { Text(snippet).font(.caption).foregroundStyle(.secondary).lineLimit(3) }
              if let publishedAt = source.publishedAt { Text(publishedAt).font(.caption2).foregroundStyle(.tertiary) }
            }.padding(.vertical, 3)
          }
          if view.sources.isEmpty && view.answer == nil { code(tool.output) }
        }
        if view.truncated { Badge(text: "结果已截断", color: .orange) }
      }
    }
  }
...
private struct Card<Content: View, Trailing: View>: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 10) {
      HStack(spacing: 8) { Image(systemName: icon).foregroundStyle(.tint); Text(title).font(.caption.weight(.semibold)).lineLimit(1); Spacer(); trailing }
      content
    }
    .padding(11)
...
private struct DiffLines: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 0) {
      if let old { ForEach(old.split(separator: "\n", omittingEmptySubsequences: false).map(String.init), id: \.self) { Text("− \($0)").foregroundStyle(.red).frame(maxWidth: .infinity, alignment: .leading) } }
      ForEach(new.split(separator: "\n", omittingEmptySubsequences: false).map(String.init), id: \.self) { Text("+ \($0)").foregroundStyle(.green).frame(maxWidth: .infinity, alignment: .leading) }
    }.font(.system(.caption, design: .monospaced)).textSelection(.enabled)
  }
}
...
private struct Badge: View {
  let text: String
  let color: Color
  var body: some View { Text(text).font(.caption2.weight(.medium)).foregroundStyle(color).padding(.horizontal, 6).padding(.vertical, 3).background(color.opacity(0.12), in: Capsule()) }
}
