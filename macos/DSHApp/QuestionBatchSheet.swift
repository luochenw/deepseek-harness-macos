...
struct QuestionBatchSheet: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 14) {
      Text("Agent 需要你的回答").font(.title2.weight(.bold))
      ScrollView {
      ...
      HStack {
        Spacer()
        Button("取消") { harness.answerQuestionBatch(question, selections: [:], custom: [:]) }.buttonStyle(.bordered)
        Button("提交") { harness.answerQuestionBatch(question, selections: selections, custom: custom) }.buttonStyle(.borderedProminent)
      }
